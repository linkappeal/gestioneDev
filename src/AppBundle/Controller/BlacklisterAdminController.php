...
//$Query="SELECT data_creazione FROM blacklist_numeri where cellulare=".$cell." ORDER BY data_creazione DESC LIMIT 1";
	 ...
	 public function blacklistAction(Request $request){
		...
		if($type=='int'){
            ...
            $QuerySelect ="SELECT id, cellulare, source_db, source_tbl, source_id FROM lead_uni where id IN (
                             select id from lead_uni where (cellulare = ".$cell." or cellulare = md5(".$cell.")))";
            ...
            $Query_All_Esterne = " select id from lead_uni_esterne where (cellulare = ".$cell." or cellulare = md5(".$cell."))";
            $em9 = $this->getDoctrine()->getManager();
