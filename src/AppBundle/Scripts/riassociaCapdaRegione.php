<?php
$RegProv=array("Abruzzo"=>"AQ", "Basilicata"=>"Pz", "Calabria"=>"CZ", "Campania"=>"NA", "Emilia-Romagna"=>"BO", "Emilia Romagna"=>"BO", "Friuli-Venezia Giulia"=>"TS", "Lazio"=>"RM", "Liguria"=>"GE", "Lombardia"=>"MI", "Marche"=>"AN", "Molise"=>"CB", "Piemonte"=>"TO", "Puglia"=>"BA", "Sardegna"=>"CA", "Sicilia"=>"PA", "Toscana"=>"FI", "Trentino-Alto Adige"=>"TN", "Umbria"=>"PG", "Valle Daosta"=>"AO", "Veneto"=>"VE");
...
try{
	$SelectAna = "select regione from lead_uni_operations where cap is null and provincia is null AND regione is not null group by regione"; // LEAD DA inviare oggi
	if (!$result = $mysqli_dest->query($SelectAna)) {
		...
		while ($row = $result->fetch_assoc()){
            ...
            $reg=$row['regione'];
            if(array_key_exists($reg,$RegProv)){
                $prov=$RegProv[$reg];
                echo "regione:".$row['regione'].", provincia: ".$prov." \n";
				...
				if (!$result2 = $mysqli_dest->query($FindCap)) {
					...
					while ($row2 = $result2->fetch_assoc()){
						...
						echo "Trovata cap: ".$row2['cap_da']."per la provincia ".$row['provincia'];
							$cap=$row2['cap_da'];
							$AddRegEProv = "UPDATE lead_uni_operations SET cap ='".$cap."', provincia='".$prov."'  WHERE regione='".$reg."' AND cap is null"; //
						ECHO $AddRegEProv." \n";
                ...
                echo "non abbiamo trovato provincia in base alla regione: ".$reg." \n";
            }
		}
	}
