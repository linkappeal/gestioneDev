...
if (!$Mresult = $mysqli_get_misc->query($SelectMiscelata)) {
	...
	while ($misc = $Mresult->fetch_assoc()){
		...
		if($limite>0 AND $limite > $alreadyMixedLeadsNumber){
			...
			//old : $NumberOfCold=floor(($CountHotNewLead*$PercentualeFredde)/(100-$PercentualeFredde));
			//per stabilire un numero di lead fredde accurato dobbia:
			//a prendere il numero totale delle lead già presenti + le calde recuperate
			$TotalNumberOfLeadsLive= $countKey+$CountHotNewLead;
			//Stabilire in base al primo dato (numero totale) quante dovrebbero esssere le fredde seguendo la percentuale
			$SupposedTotalNumberOfcold=$TotalNumberOfLeadsLive*$PercentualeFredde/100;
			//recuperare il numero di fredde dalla tabella miscelate_report
			$QueryFreddeGiaMiscelate='SELECT COUNT(*) as totalefredde FROM miscelate_light_report WHERE id_miscelata='.$MiscId.' AND  lead_type="fredda"';
			$mysqli_get_totalcold = new mysqli($DBgestione['host'],
                          $DBgestione['username'],
						  $DBgestione['password'],
                          $DBgestione['db']);

			if (!$TFresult = $mysqli_get_totalcold->query($QueryFreddeGiaMiscelate)) {
				$FreddeGiaMiscelate=0;
			}else{
				while ($TFresultloop = $TFresult->fetch_assoc()){
					$FreddeGiaMiscelate=$TFresultloop['totalefredde'];
				}
			}
			//sottrarre dal numero trovato il numero di fredde già recuperate e florare
			$NumberOfCold=floor($SupposedTotalNumberOfcold-$FreddeGiaMiscelate);
