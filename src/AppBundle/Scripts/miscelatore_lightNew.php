...
$logfile = fopen($curDir."/log/".date("Ymd")."_miscelatore_lightNew.txt", "a"); //FILE di LOG
...
//esclusiva per te:
$DBesclusivaperte =  array('host'=>'46.16.95.229', 'db'=>'esclusiv_cpl','username'=>'esclusiv_cplus', 'password'=>'HYpN%Ep!Mhxg', 'nome'=>'NAME','cognome'=>'SURNAME','cellulare'=>'PHONE1', 'data'=>'DATA');
$DBoffertesumisura=  array('host'=>'46.16.95.229', 'db'=>'offertes_cpl','username'=>'offertes_ucps', 'password'=>';2mgK6y7sZQg', 'nome'=>'NAME','cognome'=>'SURNAME','cellulare'=>'PHONE1', 'data'=>'DATA');
$DBoffertepromozioni=  array('host'=>'46.16.95.34', 'db'=>'offertep_cpl','username'=>'offertep_cpl', 'password'=>'UW[J8ScXWUJC', 'nome'=>'NAME','cognome'=>'SURNAME','cellulare'=>'PHONE1', 'data'=>'DATA');
$DBleadout=  array('host'=>'46.16.95.34', 'db'=>'leadoutl_dbb','username'=>'leadoutl_usbd', 'password'=>'A%2)8s!JTkBp','nome'=>'NAME','cognome'=>'SURNAME','cellulare'=>'PHONE1', 'data'=>'DATA');
$DBconcorso =  array('host'=>'46.254.38.170', 'db'=>'leadoutcnc_db562','username'=>'leadoutcnc_usrbd', 'password'=>'pB?HzC4GPZQi','nome'=>'nome','cognome'=>'cognome','cellulare'=>'cellulare', 'data'=>'data');
$DBgestione= array('host'=>$db_config['parameters']['database_host'], 'db'=>$db_config['parameters']['database_name'],'username'=>$db_config['parameters']['database_user'], 'password'=>$db_config['parameters']['database_password'],'nome'=>'nome','cognome'=>'cognome','cellulare'=>'cellulare', 'data'=>'data');
...
$SelectMiscelata= "select * from miscelate_light_test where attiva=1";
...
if (!$result = $mysqli_get_misc->query($SelectMiscelata)) {
	...
	while ($misc = $result->fetch_assoc()){
		...
		if($limite>0 AND $limite > $alreadyMixedLeadsNumber){
			...
			foreach($HotSources as $HotSourceName=>$HotSourceData){
				...
				if($stopLeads==0){
					...
					$selectHotDataQuery="select ".$dbvarname['nome'].", ".$dbvarname['cognome'].", ".$dbvarname['cellulare'].", ".$dbvarname['data']." from ".$HotSourceName.$KeyAlreadyPassedToQuery.$LimitQueryStringFinal;
					...
					if (!$result = $mysqli_par->query($selectHotDataQuery)) {
						...
						while ($Datas = $result->fetch_assoc()){
							...
							$NewLeads[]=array($LeadName, $LeadCognome,$LeadCellulare,$HotSourceName,$dbvarname['db'],$HotSourceEt,"calda",$LeadOriginalData);
							$CountHotNewLead++;
						}
					}
				}
			...
			if($NumberOfCold>0 AND is_array($ColdSource) AND isset($ColdSource[1]) AND in_array($ColdSource[1],$DbsNameArray) AND isset($ColdSource[2])){
				...
				if($stopLeads==0){
					...
					$QueryRecuperoFredde="select ".$dbvarname['nome'].", ".$dbvarname['cognome'].", ".$dbvarname['cellulare'].", ".$dbvarname['data']." from ".$ColdSource[0].$KeyAlreadyPassed." order by rand() ".$LimitQueryStringFinal;
					...
					if (!$result = $mysqli_par->query($QueryRecuperoFredde)) {
						...
						while ($Datas = $result->fetch_assoc()){
							...
							$LeadOriginalData=$Datas[$dbvarname['data']];
							//echo "lead datas: nome:".$LeadName."; cognome: ".$LeadCognome."; cellulare: ".$LeadCellulare . " \n\n";
							$NewLeads[]=array($LeadName, $LeadCognome,$LeadCellulare,$ColdSource[0],$ColdSource[1],$ColdSource[2],"fredda",$LeadOriginalData);
							$countCold++;
						}
			...
			if(count($NewLeads)>0){
				...
				foreach($NewLeads as $NewLead){
					...
					if ($mysqli_par->query($QueryInsertNL) === TRUE) {
						...
						$QueryInsertReportNL='insert into miscelate_light_report set id_miscelata = "'.$MiscId.'", id_lead = "'.$id_lead_mixed.'", lead_type = "'.$NL_Type.'", source_tbl = "'.$NL_Table.'", source_db ="'.$NL_Db.'", source_et ="'.$NL_Etichetta.'", data_miscelazione="'.$NL_Data_Miscelata.'", data_lead="'.$NL_OriginalData.'"';
		...
		if($stopLeads==1){
			//send mail
			$StopMiscelataQuery='update miscelate_light_test set attiva = 0 where id='.$MiscId;
			$mysqli_get_misc = new mysqli($DBgestione['host'],
                          $DBgestione['username'],
						  $DBgestione['password'],
                          $DBgestione['db']);
