...
try{
	...
	$SelectCampagne = "select id, campagna, messaggio, id_fornitore_sms, con_risposta  from campagne_sms where stato=3 AND data_lancio ='".$oggi."' LIMIT 1;"; // LEAD DA inviare oggi
...
// ***************************************************************************************				
				...
				$Cmessaggio=trim($Campagna['messaggio']);
// ***************************************************************************************				
                ...
                if(update_stato_campagna(4, $Cid)){
					 echo "Ho settato lo stato campagna a 4 <br>";
                     fwrite($logfile, "\n Ho settato lo stato campagna a 4  \n\n");
                    //controllo i dati campagna
                    //$datiCampagnaOK=1;
                    if(!strlen($Cmessaggio)>0){
                        update_stato_campagna(7, $Cid, 'errore: manca il messaggio');
                        esci();
                    }
                    if(!$IdFornitoreSms>0){
                        update_stato_campagna(7, $Cid, 'errore: manca il fornitore');
                        esci();
                    }
                    ...
                    if (!$resultFornitore = $mysqli_dest->query($SelectFornitoreSms)) {
                        ...
                        fwrite($logfile, $errorStr);
                        update_stato_campagna(7, $Cid, 'errore: il fornitore sms con id: '.$IdFornitoreSms.'non è stato trovato a db.');
                        esci();
                    }
                    ...
                    if(!strlen($ScriptLancio)>0){
                        update_stato_campagna(7, $Cid, 'errore: manca lo script di lancio del fornitore a db');
                        esci();
                    }
                    
                    //convertiamo il messaggio in gsm7 e controlliamo lunghezza
					//if($messLenght=strlen($Cmessaggio)){
/*
// ***************************************************************************************
...
*/
                    ...
                    $SelectTest="SELECT id, cellulare  FROM extraction_sms WHERE campagna_id='".$Cid."' AND test=0 AND stato_invio =0";
                    $CicloNumeri=array();
                    $Numeri=array();
                    if (!$resultNumeri = $mysqli_dest->query($SelectTest)) {
                        ...
                        fwrite($logfile, $errorStr);
                        update_stato_campagna(7, $Cid, 'errore: query di recupero numeri non andata a buon fine');
                        esci();
                    }else {
                        $countNumeri=0;
                        $countNumeriAss=0;
                        ...
                        if(count($CicloNumeri)>0){
                            ...
                            foreach($CicloNumeri as $numeri){
                                $countLanci++;
                                //print_r($numeri);
                                echo "\n Lancio n. ".$countLanci.", quantita numeri: ".count($numeri)."  \n\n";
                                fwrite($logfile, "\n Lancio n. ".$countLanci."  \n\n");
...
// ***************************************************************************************
                                include('fornitori_sms_script/'.$ScriptLancio.'_DECODIFICATO.php');
...
// ***************************************************************************************
                                ...
                                if($LancioResult['statoLancio']==1){
                                    ...
                                    if($LancioResult['UpdateNumeri']==1){
                                        ...
                                        echo "\n\n abbiamo ".count($LancioResult['UpdateNumeriDati'])." da uplodare \n\n";
                            ...
                            //inizializzo parte della query di update campagna
                            ...
                            if($lanciOk==1){
                                ...
                                $UpdateStato5 = "UPDATE campagne_sms set stato =5 ".$UpdateIdCampagnaByFornitore." WHERE id= ".$Cid ;// 
								echo "\n $UpdateStato5 \n\n";
                                if ( $mysqli_dest->query($UpdateStato5)  === TRUE ) {
                                    echo "update campagna stato a 5 RIUSCITO <br>";
                                    fwrite($logfile, "\n update campagna stato a 5 RIUSCITO.  \n\n");
                                }else{
                                    echo "Attenzione update campagna stato a 5 FALLITO <br>";
                                    fwrite($logfile, "\n Attenzione update campagna stato a 5 FALLITO.  \n\n");
                                }
                            }else{
                                //lancio fallito
                                echo "Lo stato del lancio  restituito dal forniotre è un errore. Settiamo la campagna con stato a 7 passo alla prossima campagna. Errore restituito dal fornitore: ".$LancioResult['ErroreLancio']." \n\n";
                                fwrite($logfile, "\n Non abbiamo recuperato nessun numero di test. Settiamo la campagna con stato a 7 passo alla prossima campagna . Errore restituito dal fornitore: ".$LancioResult['ErroreLancio']."  \n\n");
                                 //update lo stato a 6
                                 $erroreLancio="il fornitore ha dato KO al lancio n ".$countLanci." con il seguente errore: ".$LancioResult['ErroreLancio'];
                                $UpdateStato7 = "UPDATE campagne_sms set stato =7 ".$UpdateIdCampagnaByFornitore.", info='".$erroreLancio."' WHERE id= ".$Cid ;// 
                                if ( $mysqli_dest->query($UpdateStato7)  === TRUE ) {
                                    echo "update campagna stato a 7 RIUSCITO <br>";
                                    fwrite($logfile, "\n update campagna stato a 5 RIUSCITO.  \n\n");
                                }else{
                                    echo "update campagna stato a 7 FALLITO <br>";
                                    fwrite($logfile, "\n update campagna stato a 5 FALLITO.  \n\n");
                                }
                                esci();
                            }
                            ...
                            fwrite($logfile, "\n Non abbiamo recuperato nessun numero di test. Settiamo la campagna con stato a 6.  \n\n");
                             //update lo stato a 6
                            update_stato_campagna(7, $Cid, 'Errore: Non abbiamo recuperato nessun numero per il lancio');
                            esci();
                        }
