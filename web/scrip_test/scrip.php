...
//					echo $mail ." from ".$mo."\n";
                    ...
                    //Dobbiamo aggiungere le info riguardanti la campagna
                        ...
                        $infoCampaign = getCampagnaInfo($mysqli_dest, $db, $mail);
                        $campaign_id = $infoCampaign['id'];
                        $brand_id = $infoCampaign['brand_id'];
                        
                        if (!$campaign_id){
                            $str = " Campaign: ".$mail." has no data in campaign table. Writing campaign info... \n";
                            fwrite($logfile, date("YmdHis").$str);
                            echo $str;
                            ...
                            $str = " Found some campaign: ".$mail." data in campaign. \n";
                            fwrite($logfile, date("YmdHis").$str);
                            echo $str;
            ...
            //e tutte le lead ad esse associate
                    ...
                    $str = "A new mailoperation has been found: ".$mo." \n";
                    fwrite($logfile, date("YmdHis").$str);
                    echo $str;
        ...
        foreach($orphans as $mail){
                ...
                if(array_key_exists($mail['nome_tabella'], $arrLeadUniAllTbls[$db] )){ 
                    ...
                    if(empty($mail['tabella_master'])){
                        ...
                        $infoCampaign = getCampagnaInfo($mysqli_dest, $db, $mail['tabella_master']);
    ...
    //Se no dobbiamo aggiungere tutte le campagne e le lead contenute
    else{
        
        $str = " DB ".$db." has no current leads in lead_uni: adding them all\n";
        fwrite($logfile, date("YmdHis").$str);
        echo $str;
...
$str =  "Tempo di esecuzione: ".date("H:i:s", $end_time - $start_time)."\n";
fwrite($logfile, date("YmdHis").$str);
echo $str;
...
function getLeadUniData($mysqli){
    ...
    $strSql  = "select lead_uni.source_db as db, dbtabmo as mo, source_tbl as tbl, campagna.brand_id as brand_id, max(lead_uni.source_id) as maxid, is_active, campagna.id as idcampagna ";
    ...
    foreach($arrValues as $column => $value){
        ...
        switch(strtolower($column)){
            ...
            case 'phone1':          if(($value == 'non presente') || empty($value)) $strSql .= "cellulare  = '',";
                                    else $strSql .= "cellulare  = '".addslashes($value)."',";
                                    break;                                    
